...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!finishedLoading && !locationState.loadingGeoLocation) {
      setZoom(
        locationState.geoLocationEnabled || locationState.loadingGeoLocation
          ? 7
          : 2
      );
      ...
      !currentUserState.isLoggedIn &&
        openSnackbar(
          "Tip: zoom in and out to find more locations.",
          "info",
          25000
        );
    }
    ...
    >
      ...
      {allLocations.map((loc) => (
        <Circle
          ...
          color={
            nearLocationData && nearLocationData.includes(loc.id)
              ? secondary
              : primary
          }
          radius={size}
      ...
      {!loading &&
        nearLocationData.length === 0 &&
        locationState.geoLocationEnabled &&
        youAreHereMarker()}

      <CustomZoomControl setZoom={setZoom} zoom={zoom} />
    </Map>
  );
}
