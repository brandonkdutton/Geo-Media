...
const PostStack = (props) => {
    ...
    const postsLoadedForThisLocation = ((postsState.chachedLocationId === locationState.current) || locationState.current === null);
    const replyingToRootPost = ((replyingToState === null) && canPostToThisLocation) 
        ...
        return (
            <Grid container direction="column" justify="flex-start" alignItems="flex-end" spacing={1} className={classes.marginStyle}>
                ...
                <Grid item style={{ 'width': `100%`, 'marginBottom': (replyingToRootPost ? '0px' : '-8px') }}>
                    <Collapse in={postsLoadedForThisLocation} timeout="auto" unmountOnExit>
                        <WriteCard
                            postObj={{ post_id: -1 }}
                        />
                    </Collapse>
